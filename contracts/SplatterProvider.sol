...
contract SplatterProvider is IAssetProvider, IERC165, Ownable {
  ...
  function generateSVGPart(uint256 _assetId) external pure override returns(string memory svgPart, string memory tag) {
    ...
    // debug (seed, length) = seed.randomize(length, 50); // +/- 50%
    ...
    for (uint i = 0; i < count; i++) {
      uint degree = 100;
      // debug (seed, degree) = seed.randomize(degree, 90);
      degrees[i] = total;
      total += degree;
    }
    ...
    for (uint i = 0; i < count; i++) {
      {
        ...
        if (i % 3 == 0) {
          ...
          points[j].r = 566;
          j++;
          points[j].x = int32(512 + (angle - 100).cos() * int(r1 + extra) / 0x8000);
          points[j].y = int32(512 + (angle - 100).sin() * int(r1 + extra)  / 0x8000);
          points[j].c = true;
          points[j].r = 566;
          j++;
          points[j].x = int32(512 + (angle + 100).cos() * int(r1 + extra)  / 0x8000);
